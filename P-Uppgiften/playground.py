...
class Playground:
    ...
    def mouseInput(self):
        ...
        if (self.mouse.leftButton == False):
            self.clickSwitch = False

        if (self.mouse.rightButton):
            for cell in self.cells:
                self.screen.canvas.itemconfigure(cell.rect, state='hidden')
        else:
            for cell in self.cells:
                self.screen.canvas.itemconfigure(cell.rect, state='normal')
                
        if (self.mouse.rightButton == False):
            self.clickSwitch2 = False
